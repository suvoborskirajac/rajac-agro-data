...
MIN_PARCELS_WITH_DATA_PCT = 60.0    # accept acquisition if ≥X% of parcels have a usable signal
MIN_VALID_PX_PER_PARCEL = 10        # parcel counts as "with data" if has at least this many valid px
...
def evaluate_acquisition(acq: Acquisition, parcels_fc: Dict[str, Any], region: ee.Geometry,
    ...
    # Per-parcel coverage based acceptance — accept acquisition only if enough parcels have data
    n_parcels = len(parcels_fc["features"])
    parcels_with_data = sum(1 for p in parcel_results if p.get("valid_pixels", 0) >= MIN_VALID_PX_PER_PARCEL)
    parcels_with_data_pct = (parcels_with_data / n_parcels * 100.0) if n_parcels else 0.0
    ...
    return {
        ...
        "meta": {
            ...
            "parcels_with_data": parcels_with_data,
            "parcels_total": n_parcels,
            "valid_coverage_pct": round(parcels_with_data_pct, 1),
            "scale_m": scale_m,
...
def build_trends_and_alerts(parcels_fc: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    if not daily_records:
        return (
            {"ok": True, "trends": {}, "note": "Нема дневних снимака.",
             "updated_at": datetime.now(timezone.utc).isoformat(timespec="seconds").replace("+00:00", "Z"),
             "latest_acquisition": None, "acquisitions_count": 0, "month": date.today().strftime("%Y-%m")},
            {"ok": True, "active": [], "count": 0,
...
def main() -> int:
    ...
    for acq in acquisitions:
        ...
        if valid_pct < MIN_PARCELS_WITH_DATA_PCT:
            log(f"  rejected {acq.id}: only {valid_pct}% of parcels with data (need ≥{MIN_PARCELS_WITH_DATA_PCT}%)")
            continue
        write_json(out_path, result)
        processed += 1
        log(f"  OK wrote {out_path.name} (parcels with data: {valid_pct}%)")
